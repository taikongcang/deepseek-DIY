...
| **N33** | **用量管理（🔴 讨论中）** | 上次是 fork 第三方 `dsh-cost-meter` v1.6.12（改 5 文件 / `src/client.js` 439 行：圆环 88px、峰谷四值样式、新增摆锤视图、i18n）。**09-01 20:32 用户已拍板 9 项功能清单**（费用金额体系 / 价格表 / 峰谷计价 / 官方价格一键同步 / 预算图框 / 官方余额查询 / 安装前历史导入 / 峰谷切换弹窗 / 26 周热图），停在"逐项讲解待审"。**现状：app 里没有它**。**三个隐患**：① 439 行改动无 `.git` ② `修改代码\cost-meter\` 误套一层重复目录 ③ 版本号仍写 1.6.12。**等用户定性"上次的问题是什么"**，再选 A 继续 fork · B 自研 · C 用上游原版。参考：DSH **自带**用量显示（TTFT / tok/s / 轮次步数 / 上下文压力 / 组成），与 cost-meter **互补**（内置管 token 与上下文，它管"钱"和历史） |
