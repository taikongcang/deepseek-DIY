...
| **6** | `I:\deepseekharness` 野目录 | **0 MB**（只有一个空的 `AI股票` 目录） | — | ⛔ **用户 2026-09-15 20:23 定：永久标记「不需要管」，永远不处理、不再询问** |
| **7** | 2026-09-15 查证 UI 归属时我在 `I:\` 根留下的探测文件 | 小 | 先归档到 `.workbuddy\_archive\` | ✅ **已完成**（2026-09-15 20:3x 复核：原记的 **14 个 `_q*.txt` 实测早已全部不在**；另发现 **9 个新产生的** `_g1..3` / `_r1..6`（16:45–16:55），已按用户指示清出根目录、归档到 `.workbuddy\_archive\agent-probe-2026-09-15\`） |

**批 7 原明细（2026-09-15 20:3x 复核：这批早已清完，`I:\` 根实测 0 命中）**：`_q1_old.txt` `_q1b.txt` `_q1c.txt` `_q2_history.txt` `_q3_aa.txt` `_q3_aa2.txt` `_q4.txt` `_q5.txt` `_q5b.txt` `_rb.txt` `_arch.txt` `_tail.txt` `_core_loc.txt` `_verify_after_clean.txt`

**批 7-补（2026-09-15 新增，已清理并归档）**：`_g1.txt` `_g2.txt` `_g3.txt` `_r1.txt` `_r2.txt` `_r3.txt` `_r4.txt` `_r5.txt` `_r6.txt`（共 9 个 / 61 KB；内容 = gh CLI 检查、目录体积、提交身份、i18n yaml 全文、safe-delete 结果、blob 哈希、测试输出、文件大小）
...
删掉的东西**都进了回收站**，所以回收站里压着 **8.8 GB（I 盘）**。

> ⚠️ **2026-09-15 20:3x 复核更新**：原记 **7.03 GB**，本次**现场实测** `du -sh I:\$RECYCLE.BIN` = **8.8 GB**（按铁律 8：不沿用旧快照，以实测为准）。
> 用户 2026-09-15 20:23 定：**清理（清空回收站）**。因该操作**不可逆**，执行前会再单独确认一次。

| 指标 | 数值 |
|---|---|
| `I:\` 可用空间（2026-09-15 20:3x 实测） | **484 GB** |
| `E:\` 可用空间（2026-09-15 20:3x 实测） | **453 GB** |
| `I:\$RECYCLE.BIN` 占用（**2026-09-15 20:3x 实测**） | **8.8 GB** |

→ 看起来"可用空间没变多"，是因为**回收站占着**。**要真正拿回约 8.8 GB，需要清空回收站。**
