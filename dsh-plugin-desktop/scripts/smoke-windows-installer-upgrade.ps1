...
$taskExistingInstalls = @(Get-ItemProperty $taskUninstallRoots -ErrorAction SilentlyContinue | Where-Object {
  $_.DisplayName -match '^deepseekharness'
})

if ($taskExistingProcesses.Count -gt 0 -or $taskExistingInstalls.Count -gt 0) {
  throw 'Refusing to run while an existing deepseekharness process or installation is present.'
}
...
$taskUserData = Join-Path $env:APPDATA 'deepseekharness'
$taskDshHome = Join-Path $taskRoot 'dsh-home'
$taskActiveRunMarker = Join-Path $taskUserData 'crash-evidence\active-run.json'
$taskAppPath = Join-Path $taskInstallRoot 'deepseekharness.exe'
$taskUninstallerPath = Join-Path $taskInstallRoot 'Uninstall deepseekharness.exe'
if (Test-Path -LiteralPath $taskActiveRunMarker) {
  throw 'Refusing to overwrite an existing deepseekharness active run marker.'
}
$taskResult = [ordered]@{
...
function Wait-TaskProcess([bool]$taskShouldExist, [int]$taskTimeoutSeconds = 30) {
  ...
  throw "Timed out waiting for deepseekharness process state: shouldExist=$taskShouldExist"
}
...
try {
  ...
  $taskRemainingShortcuts = @($taskShortcutRoots | ForEach-Object {
    Get-ChildItem -LiteralPath $_ -Filter '*deepseekharness*' -Recurse -ErrorAction SilentlyContinue
  })
