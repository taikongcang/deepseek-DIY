...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `deepseekharness-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'deepseekharness.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing deepseekharness.exe: ${portablePath}`)
  }
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:deepseekharness.exe`,
  )
  return portablePath
}
