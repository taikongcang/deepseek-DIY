  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'deepseekharness' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `deepseekharness-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('uses the independent macOS compatibility frame, Dock icon, and template tray image', async () => {
    ...
    expect(electron.browserWindows[0]?.accessibleTitle).toBe('deepseekharness')
  ...
  it('rebuilds ordered effect-scoped tray contributions without replacing native commands', async () => {
    ...
    expect(labels).toEqual([
      'Open deepseekharness', undefined,
  ...
  it('renders contributed radio submenus in their own profile section', async () => {
    ...
    }>).find(item => item.label === 'deepseekharness')
  ...
  it('shows native recovery when the renderer Loader reports a failed plugin', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      ...
      buttons: ['Open R9', 'Restart deepseek harness', 'Dismiss'],
    }))
  ...
  it('requires Desktop dialog confirmation before an ordinary restart', async () => {
    ...
    expect(restart).not.toHaveBeenCalled()
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      type: 'question', title: 'Restart deepseek harness', buttons: ['Restart', 'Cancel'], defaultId: 1, cancelId: 1,
    }))
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/deepseekharness-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    const release = runtime.schedule(spec)
    await runtime.mountScheduled()
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
        title: 'deepseek harness Is Up to Date',
    ...
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/deepseekharness-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/deepseekharness-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/deepseekharness-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
        title: 'deepseek harness Update Downloaded',
        buttons: ['OK'],
      }),
    )
  ...
  it('starts the downloaded Windows installer visibly before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\deepseekharness-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\deepseekharness-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\deepseekharness-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\deepseekharness-2.1.0-windows.exe',
    })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\deepseekharness-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/deepseekharness-2.1.0-mac.dmg',
    })
