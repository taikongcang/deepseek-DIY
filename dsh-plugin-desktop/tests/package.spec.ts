...
describe('published package surface', () => {
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.productName).toBe('deepseekharness')
    expect(manifest.build?.appId).toBe('deepseekharness.diy')
    ...
    expect(manifest.build?.win?.artifactName).toBe('deepseekharness-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      shortcutName: 'deepseekharness',
      uninstallerIcon: 'build/app-icon.ico',
      useZip: false,
      artifactName: 'deepseekharness-${version}-${arch}-Setup.${ext}',
    })
