/** Headless, confirmation-gated downloads for this build's own installers. */
...
import {
  ...
  DESKTOP_UPDATE_SOURCE_DIRECTORY,
  parseSemVer,
...
/**
 * Installer filename published inside {@link DESKTOP_UPDATE_SOURCE_DIRECTORY}.
 *
 * The stable Windows name mirrors `nsis.artifactName` in
 * `dsh-plugin-desktop/package.json`, so a freshly packaged installer can be
 * copied into the source directory without renaming. macOS only keeps the
 * platform contract total: this build publishes no macOS artifact because the
 * `mac` target is `dir`.
 */
export function desktopUpdateArtifactName(
  platform: DesktopDownloadPlatform,
  version: string,
  channel: DesktopReleaseChannel = 'stable',
): string {
  const product = channel === 'beta' ? 'deepseekharness-Beta' : 'deepseekharness'
  return platform === 'win32'
    ? `${product}-${version}-x64-Setup.exe`
    : `${product}-${version}-mac.dmg`
}

/** Local installer URL that records one user-confirmed download. */
export function desktopUpdateDownloadUrl(
  ...
  return pathToFileURL(
    join(DESKTOP_UPDATE_SOURCE_DIRECTORY, desktopUpdateArtifactName(platform, version, channel)),
  ).href
}
