...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'deepseek harness 有可用更新', body: `版本 ${version} 已可下载。打开 deepseek harness 即可继续。` }
    : { title: 'deepseek harness Update Available', body: `Version ${version} is ready to download. Open deepseek harness to continue.` }
}
