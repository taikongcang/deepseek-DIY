/** Headless version checks against this build's own local release source. */
...
/**
 * Local directory holding this build's own release artifacts.
 *
 * This build publishes its own updates instead of following the community
 * release service: `version.json` carries the latest version and the packaged
 * installers sit beside it. A system-level `DSH_HOME` must not redirect it, so
 * the path is absolute and machine-specific by design.
 */
export const DESKTOP_UPDATE_SOURCE_DIRECTORY = 'I:\\deepseekharness更新DIY\\发布'

/**
 * Local endpoint returning the latest version published by this build.
 * `pathToFileURL` emits the percent-encoded form, which is what Electron
 * `net.fetch` expects for non-ASCII directory names.
 */
export const DESKTOP_VERSION_ENDPOINT =
  pathToFileURL(join(DESKTOP_UPDATE_SOURCE_DIRECTORY, 'version.json')).href
