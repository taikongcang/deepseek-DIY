...
Market 安装不会创建 receipt、checkpoint、重试、清理或回滚 operation。结果状态由 deepseek harness 普通的 Profile checkpoint 统一覆盖。
...
1. deepseek harness 读取当前 Profile 的 `dependencies` 和 `dsh.profile.bundles`。
...
自动 preview 不可用时，Host 可以根据标准化身份构造一条有界且只用于展示的 npm 命令。**打开 R9**只打开终端，不会提交 package 命令、路径或 Profile，也不会执行修改。
...
pnpm 进程失败时，Host 只保留有上限的 stdout/stderr 尾部，将该诊断写入 deepseek harness 日志，并返回给本地失败弹窗。弹窗显示已验证 package 对应的命令，也可以打开 R9，但不会粘贴、执行或静默重试该命令。
