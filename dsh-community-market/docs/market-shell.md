...
Status: delivered and built into deepseek harness.
...
deepseek harness Profile inventory -> Installed view -> opaque bundleId -> pnpm remove
```
...
deepseek harness inventory reads direct Profile dependencies and bundles. It therefore includes plugins installed by this Market, other markets, and the DSH CLI. Product-owned bundles are read-only. Other direct plugin dependencies receive an opaque generation-scoped `bundleId` and offer uninstall only.
...
Browsing remains available when deepseek harness package capabilities are absent. A source failure never blocks deepseek harness startup. Package-operation errors do not trigger automatic cleanup or rollback. deepseek harness's three healthy-start Profile checkpoints are the single recovery mechanism, and restoration remains an explicit Recovery-page action.
