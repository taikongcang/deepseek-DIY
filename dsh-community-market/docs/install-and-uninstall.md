...
Market installation creates no receipt, checkpoint, retry, cleanup, or rollback operation. deepseek harness's ordinary Profile checkpoints cover the resulting state.
...
1. deepseek harness reads the active Profile's `dependencies` and `dsh.profile.bundles`.
...
If automatic preview is unavailable, the Host may construct a bounded display-only npm command from normalized identity. **Open R9** opens the terminal only; it sends no package command, path, or Profile and performs no mutation.
...
For a pnpm process failure, the Host retains only a bounded tail of stdout and stderr. It records that diagnostic in the deepseek harness log and returns it to the local failure dialog. The dialog shows the verified package command and can open R9, but never pastes, executes, or silently retries it.
