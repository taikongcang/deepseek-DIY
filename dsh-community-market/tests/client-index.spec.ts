...
describe('community market client registration', () => {
  ...
  it('registers locale, styles, sidebar launcher, and shell overlay effects', () => {
    ...
    // DIY: the settings > plugins > market tab was removed; the sidebar launcher is the
    // single market entry, so there are exactly two injected slots left.
  ...
  it('projects all slot registrations with the market identity, locale, and shared view store', () => {
    ...
    const [launcher, overlay] = test.registrations.map(value => value.spec)
