...
describe('community market browser plugin', () => {
  ...
  // DIY: the settings > plugins > market tab was removed, so only the sidebar launcher
  // and its shell overlay remain.
  it('registers one shared Market surface in the sidebar and overlay slots without fetching', () => {
    const b = bench()
    for (const name of ['sidebar.footer.action', 'shell.overlay']) b.declare(name)
  ...
  it('follows late declaration and declaration reload for both entries', () => {
    const b = bench()
    b.apply()
    const names = ['sidebar.footer.action', 'shell.overlay'] as const
