...
import {
  AWESOME_LIST_ADAPTER_ID,
  AWESOME_LIST_DATA_URL,
  AWESOME_LIST_KEY,
  AWESOME_LIST_LEGACY_ADAPTER_ID,
  AWESOME_LIST_PROVIDER_ID,
  AWESOME_LIST_PUBLIC_ENDPOINT,
  awesomeListAdapter,
} from '../adapters/awesome-list.js'
...
export const BUILT_IN_PROVIDERS: readonly BuiltInProviderDefinition[] = [
  ...
  {
    key: AWESOME_LIST_KEY,
    name: 'Awesome DSH Plugins',
    description: '社区精选目录（awesome-dsh-plugin），数据内置打包、离线可浏览，安装源钉死 GitHub commit。目录收录不代表插件经过审核或推荐。',
    providerId: AWESOME_LIST_PROVIDER_ID,
    adapterId: AWESOME_LIST_ADAPTER_ID,
    endpoint: AWESOME_LIST_PUBLIC_ENDPOINT,
    attribution: {
      name: 'Awesome DSH Plugins',
      url: AWESOME_LIST_DATA_URL,
      notice: 'Community-curated catalog snapshot bundled with the app; installs are pinned to immutable GitHub commits.',
    },
    partnership: false,
  },
]
...
const adapters = new Map<string, CatalogAdapter>([
  ...
  [awesomeListAdapter.adapterId, awesomeListAdapter],
  // 旧版「DSH Marketplace」记录里的 adapterId：升级安装后自动切到 awesome 目录
  [AWESOME_LIST_LEGACY_ADAPTER_ID, awesomeListAdapter],
])
