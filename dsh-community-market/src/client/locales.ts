export const zh = {
  ...
  manualInstallBody: '以下命令由 deepseek harness 根据插件信息生成，可能与仓库中的说明不同。请先检查源码，再复制到 R9 执行。',
  installCommand: '安装命令',
  ...
  terminalError: '无法打开 R9，请从应用菜单手动打开后再复制命令。',
  restartError: '无法请求重启 deepseek harness，请稍后手动重启。',
  ...
  desktopRequiredTitle: '需要 deepseek harness',
  desktopUnavailable: '安装和卸载只在 deepseek harness 中可用；你仍然可以浏览插件目录。',
  ...
  installFailedBody: 'deepseek harness 未能完成自动安装。你可以查看详细输出，或前往 R9 手动安装。',
  installFailureTerminalHint: '自动安装失败后不会再次使用同一个确认请求。请检查下面的输出；如需继续，可以打开 R9 并手动执行安装命令。',
  failureOutput: '详细输出',
  noFailureOutput: '没有收到 package manager 的额外输出。请查看 deepseek harness 日志了解更多信息。',
  plugin: '插件',
  package: 'npm 包',
  exactVersion: '精确版本',
  profile: '当前配置',
  ...
  confirmInstallBody: '请确认 deepseek harness 验证的 npm 包、版本和目标配置。',
  ...
  restartAfterOperation: '操作完成后需要重启 deepseek harness，改动才会生效。',
  disabledRestartRequired: '此插件保留了未加载状态；插件市场仍只提供卸载。',
  installComplete: '插件安装完成',
  uninstallComplete: '插件卸载完成',
  restartRequiredTitle: '需要重启 deepseek harness',
  restartRequiredBody: '请重启 deepseek harness，让当前配置加载最新的插件状态。',
  restartLater: '稍后重启',
  restartNow: '立即重启',
  restarting: '正在重启...',
  done: '完成',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  readOnly: 'This plugin cannot currently be installed automatically in deepseek harness. You can still review its details and source repository.',
  checkingInstallMethod: 'Checking available installation methods...',
  manualInstallTitle: 'Manual installation',
  manualInstallBody: 'deepseek harness generated this command from the plugin information, so it may differ from the repository instructions. Review the source first, then copy it into R9.',
  ...
  terminalError: 'R9 could not be opened. Open it from the application menu, then copy the command manually.',
  restartError: 'deepseek harness could not be restarted. Restart it manually when convenient.',
  installableBody: 'This view shows source entries with an npm install target. Before installation, deepseek harness resolves npm latest and confirms that it is a DSH plugin.',
  ...
  desktopRequiredTitle: 'deepseek harness is required',
  desktopUnavailable: 'Install and uninstall are available only in deepseek harness. You can still browse the catalog.',
  ...
  installFailedBody: 'deepseek harness could not complete the automatic installation. Review the detailed output or continue manually in R9.',
  installFailureTerminalHint: 'deepseek harness will not reuse the same confirmation after a failed automatic install. Review the output below; to continue, open R9 and run the install command manually.',
  failureOutput: 'Detailed output',
  noFailureOutput: 'The package manager returned no additional output. Check the deepseek harness logs for more information.',
  plugin: 'Plugin',
  package: 'npm package',
  ...
  confirmInstallBody: 'Review the npm package, version, and target profile verified by deepseek harness.',
  ...
  restartAfterOperation: 'Restart deepseek harness after this operation for the change to take effect.',
  ...
  restartRequiredTitle: 'Restart deepseek harness',
  restartRequiredBody: 'Restart deepseek harness so the current profile loads its updated plugin state.',
