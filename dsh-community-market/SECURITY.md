...
- The Renderer submits source/item identities for install or an opaque deepseek harness `bundleId` for uninstall; it never chooses an arbitrary package name at execution time.
...
- Market creates no install receipt, install-specific snapshot, retry, cleanup, or rollback. Recovery is owned by deepseek harness's unified three-slot healthy-start checkpoints.
- A successful mutation may issue a short-lived one-shot restart grant. Restart remains an explicit user action.
- **Open R9** carries an empty body and only opens deepseek harness's terminal; it never pastes or executes a displayed command.
...
Exactly one source is selected for browsing. Source failure never silently selects a fallback, changes the active Profile, or blocks deepseek harness startup.
